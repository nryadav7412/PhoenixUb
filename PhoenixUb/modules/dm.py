...
async def moviefilter(event):
    if not event.sender.bot:
        if len(event.raw_text)>15:
            ...
            count=0
            async for msg in phoenixub.iter_messages(-1001589866631,reverse=True,search=srch):
                if msg.video:
                    rslts+='\n➥'+f'''__https://t.me/c/1589866631/{msg.id}__'''
                    count+=1
            if count>0:
                try:
...
async def semd(e):
     ...
     async for msg in phoenixub.iter_messages(e.chat_id, reverse=True , offset_id=from_id):
          ...
          await phoenixub.send_message(-1001589866631 , msg)
